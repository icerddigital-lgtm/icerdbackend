// ============================================================================
// ICERD — Middlewares de sécurité
//
// INSTALLATION :
//   cd backend && npm install helmet express-rate-limit compression
//
// UTILISATION dans src/server.js — remplacer :
...
//
//   Puis TOUT À LA FIN, après toutes les routes :
//     app.use(gestionnaire404);
//     app.use(gestionnaireErreurs);
//     const serveur = app.listen(PORT, () => { ... });
//     arretPropre(serveur, pool);
//
// UTILISATION dans src/routes/auth.js — appliquer le limiteur SUR la route
// elle-même (pas via app.use sur un préfixe, sinon Express tronque le chemin) :
//     import { limiteurConnexion } from '../middleware/securite.js';
//     r.post('/connexion', limiteurConnexion, async (req, res, next) => { ... });
//
// UTILISATION dans src/routes/upload.js (facultatif) :
//     import { limiteurUpload } from '../middleware/securite.js';
//     r.post('/', limiteurUpload, ...);
// ============================================================================
...
// Connexion : 10 tentatives ratées par quart d'heure et par IP (anti force brute)
...
/* ---------------------------------------------------------------------------
 * 404 : chemin d'API inexistant (renvoie du JSON, pas du HTML)
 * ------------------------------------------------------------------------- */
